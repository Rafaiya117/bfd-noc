...
$mdata = $db->select('SELECT * from users where `id` = ? AND `forgot_password` = ? limit 1; ', $user_id, $session_id);
if(empty($mdata)){
    error_try_again();
}
...
if (!empty($_POST)) { 
   ...
   if($_POST['password'] === $_POST['repassword']){
      ...
      $updt = $db->action('UPDATE users SET `password`= md5(?) , `forgot_password` = LEFT(UUID(), 8)  WHERE `id`= ? limit 1;', $_POST['repassword'], $user['id']);
