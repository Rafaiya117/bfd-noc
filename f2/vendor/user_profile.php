...
pg_header();
show_banner('home', 'My Profile :: ' . $user_info['name']);
pg_topnavbar();
// pg_navbar2();
breadcrumbs(' > ');
...
$sign = '<small class="error">Signature not uploaded. </small>';
$nid_copy = '<small class="error">NID is not Uploaded. </small>';
$profile_image = '<small class="error">NID is not Uploaded. </small>';
...
if($user_info['profile_image'] != ''){ 
    $profile_image = $img_cap . $user_info['profile_image']. '" />';
 }
...
<h4> Applicant / Company Profile </h4>
    <div style="float:right;position:relative;top:-40px">
        <a href="vendor_signature.php" class="btn btn-primary"> Update Files </a>
        <a href="resetPassword_field.php" class="btn btn-primary"> Reset Password </a>
        ...
        <thead>
            ...
            <?php 
                ...
                <th scope="col">
                 ...
                 if($user_info['purpose'] === 'Commercial'){
                    // displayfile($user_info['licence_copy'], 'licence_copy');
                    _lvtb('licence_copy', displayfile($user_info['licence_copy']));
                 }
                 ?>
                </th>
            </tr>
                ...
                _lvtb('Profile Photo', $profile_image);
...
foreach ($data as $row) {
        // $link = 'full_details.php?id=' . $row['id'];
        if($row['memo_id']==''){
