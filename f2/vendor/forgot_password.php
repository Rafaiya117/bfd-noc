...
if(!empty($_POST['email'])){
    ...
    $member_data = $db->select('SELECT * from vendor where `email` = ?',$_POST['email'])[0];
    //print_r($member_data);
    $id = $member_data['id'];
        ...
        if (!$member_data) {
            ...
            $member_data = $db->select('SELECT * FROM member WHERE id=?', $member_data['id'])[0];
            $baseUrl= BASEURL;
            $resetLink = $baseUrl . '/f2/vendor/resetPassword_from_email_link.php?user_id=' . $member_data['id'].  '&SESSION_VALID=' .$member_data['forgot_password'];
